-- Add 'declining' to the trends status check constraint
ALTER TABLE public.trends
  DROP CONSTRAINT IF EXISTS trends_status_check;
ALTER TABLE public.trends
  ADD CONSTRAINT trends_status_check
  CHECK (status IN ('watchlist', 'rising', 'active', 'declining', 'inactive'));

-- Update the nearby stores function to include declining trends
...
as $$
  ...
  inner join public.trends t
    on t.id = s.trend_id
   and t.status in ('rising', 'active', 'declining')
