...
def _extract_accessible_text(element: BeautifulSoup | Any) -> str:
    if not element:
        return ""

    text_parts = [element.get_text(" ", strip=True)]
    for attr_name in ("alt", "title", "aria-label"):
        attr_value = element.get(attr_name)
        if attr_value:
            text_parts.append(str(attr_value))

    for child in element.select("[alt], [title], [aria-label]"):
        for attr_name in ("alt", "title", "aria-label"):
...
async def _crawl_cu_fresh_food(
    client: httpx.AsyncClient,
    source: NewProductSourceDefinition,
) -> list[ParsedNewProduct]:
    products: list[ParsedNewProduct] = []
...
async def _crawl_gs25_fresh_food(
    client: httpx.AsyncClient,
    source: NewProductSourceDefinition,
) -> list[ParsedNewProduct]:
    products: list[ParsedNewProduct] = []
...
async def _crawl_html_badge_menu(
    ...
    for item in soup.select(item_selector)[:scan_limit]:
        ...
        badge_label = _extract_accessible_text(new_label).upper()
        if not new_label or badge_text not in badge_label:
            continue
...
_PARSER_HANDLERS: dict[str, Any] = {
    ...
    "cu_fresh_food": _crawl_cu_fresh_food,
    "gs25_fresh_food": _crawl_gs25_fresh_food,
    "seven_eleven_fresh_food": _crawl_seven_eleven_fresh_food,
