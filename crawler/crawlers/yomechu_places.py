...
def merge_live_place(
    ...
    trend_names = trend_lookup.get(key, existing_place.get("trend_names", []) if existing_place else [])
...
async def find_yomechu_candidates(
    ...
    if category_slug != "all" and len(requested) < 3:
        requested = await fetch_category_places(CATEGORY_CONFIG["all"]["group_code"], lat, lng, radius_m)
        used_fallback = True

    if not requested:
        raise YomechuNoResultsError("No nearby places found")
    ...
    if not merged_places:
        raise YomechuNoResultsError("No candidates after filtering")
...
async def spin_yomechu(
    ...
    if not candidates:
        raise YomechuNoResultsError("No candidates available")
...
async def refresh_recent_yomechu_ratings() -> dict[str, int]:
    ...
    for row in list_recent_yomechu_places(settings.YOMECHU_ENRICH_BATCH_SIZE):
        ...
        if row.get("rating") is not None and last_enriched and last_enriched > now - timedelta(days=1):
            continue
        if last_enriched and last_enriched > now - timedelta(hours=settings.YOMECHU_ENRICH_INTERVAL_HOURS):
            continue
